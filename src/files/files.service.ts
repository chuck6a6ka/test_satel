...
export class FilesService {
    ...
    async create(createFileDto: CreateFileDto, files: FileInfoDto) {
        try {
             ...
             */
            fs.appendFileSync(DATABASE_FILE.path, JSON.stringify(fileDatabaseInfo) + 'NEXT')
            /**
             * 2. Сохранили информацию о полученном файле
             */
            fs.writeFileSync(path.join(DATABASE_FILE.filesFolderPath, String(fileId) + '.txt'), JSON.stringify(fileInfo))
    ...
    async getOneFile(id: string){
        try{
            const fileFromDatabase = JSON.parse(fs.readFileSync(path.join(DATABASE_FILE.filesFolderPath, id + '.txt')).toString('utf-8'))
